...
void QueueLoggerSampling::doNextEvent()
{
	...
	if (!_seenQueueInD)
	{ // queue size hasn't changed in the past D time units
...
void QueueLoggerSampling::logQueue(Queue &queue, QueueEvent ev, Packet &pkt)
{
	if (_queue == NULL)
		_queue = &queue;
	assert(&queue == _queue);
...
MemoryLoggerSampling::MemoryLoggerSampling(simtime_picosec period,
										   EventList &eventlist) : EventSource(eventlist, "MemorySampling"), _period(period)
{
	eventlist.sourceIsPendingRel(*this, 0);
}
...
SinkLoggerSampling::SinkLoggerSampling(simtime_picosec period,
									   ...
									   int event_type) : EventSource(eventlist, "SinkSampling"), _last_time(0), _period(period),
...
TcpSinkLoggerSampling::TcpSinkLoggerSampling(simtime_picosec period,
											 EventList &eventlist) : SinkLoggerSampling(period, eventlist, Logger::TCP_SINK, TcpLogger::RATE)
{
}
...
NdpSinkLoggerSampling::NdpSinkLoggerSampling(simtime_picosec period,
											 EventList &eventlist) : SinkLoggerSampling(period, eventlist, Logger::NDP_SINK, NdpLogger::RATE)
{
